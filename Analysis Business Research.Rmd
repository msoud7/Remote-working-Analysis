...
data <- data %>%
  mutate(
    gender = case_when(
      gender == 1 ~ 1,  # Male
      gender == 2 ~ 2,  # Female
      gender %in% c(3, 4) ~ 3  # Combine Non-binary and Prefer not to say into Other
    ),
...
ggplot(data = data, mapping = aes(x = experience)) +
...
```{r education level}
ggplot(data = data, mapping = aes(x = factor(education_level,
                                              levels = c(1,2,3,4,5), 
                                              labels = c("1 Primary", 
                                                         "2 Secondary", 
                                                         "3 Bachelor's", 
...
```{r employment type}
ggplot(data = data, mapping = aes(x = factor(employment_type,
                                              levels = c(1,2,3,4,5, 6), 
                                              labels = c("1 Full-time", 
                                                         "2 Part-time", 
                                                         "3 Internship", 
                                                         "4 Temporary", 
                                                         "5 Freelance",
...
model_H21 <- lm("WFC_score ~ RW + age + experience + gender + education_level + employment_type", data = data)
...
model_H22 <- lm("FWC_score ~ RW + age + experience + gender + education_level + employment_type", data = data)
...
model_H23 <- lm("Conflict_score ~ RW + age + experience + gender + education_level + employment_type", data = data)
...
mediation_model <- '
  # Direct effect
  WLB_score ~ c*RW + d1*age + d2*gender + d3*education_level + d4*employment_type

  
  # Mediators
  WFC_score ~ a*RW  # Remote Work to Work-Family Conflict
  FWC_score ~ b*WFC_score  # Work-Family Conflict to Family-Work Conflict
  
  # Indirect effects
  indirect_WFC := a*c  # RW to WLB_score through WFC_score
  indirect_FWC := a*b*c  # RW to WLB_score through WFC_score to FWC_score
...
# Fit the model
fit_mediation <- sem(mediation_model, data = data)

# Review the results
summary(fit_mediation, fit.measures = TRUE, standardized = TRUE)

# Check for indirect effects
parameterEstimates(fit_mediation, standardized = TRUE)
